...
class ZhipuAIEmbeddings(BaseModel, Embeddings):
    ...
    deployment: Optional[str] = model
    # TODO: Move to AzureOpenAIEmbeddings.
    zhipuai_api_version: Optional[str] = Field(default=None, alias="api_version")
    """Automatically inferred from env var `OPENAI_API_VERSION` if not provided."""
    # to support Azure OpenAI Service custom endpoints
    ...
    """Base URL path for API requests, leave blank if not using a proxy or service 
        emulator."""
    # to support Azure OpenAI Service custom endpoints
    zhipuai_api_type: Optional[str] = None
    # to support explicit proxy for OpenAI
    ...
    """Automatically inferred from env var `OPENAI_API_KEY` if not provided."""
    zhipuai_organization: Optional[str] = Field(default=None, alias="organization")
    """Automatically inferred from env var `OPENAI_ORG_ID` if not provided."""
    allowed_special: Union[Literal["all"], Set[str]] = set()
    disallowed_special: Union[Literal["all"], Set[str], Sequence[str]] = "all"
    chunk_size: int = 512
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        if values["zhipuai_api_type"] in ("azure", "azure_ad", "azuread"):
            default_api_version = "2023-05-15"
            # Azure OpenAI embedding models allow a maximum of 16 texts
            # at a time in each batch
            # See: https://learn.microsoft.com/en-us/azure/ai-services/openai/reference#embeddings
            ...
            default_api_version = ""
        values["zhipuai_api_version"] = get_from_dict_or_env(
            values,
            "zhipuai_api_version",
            "ZHIPUAI_API_VERSION",
            default=default_api_version,
        ...
        if not values.get("async_client"):
            values["async_client"] = zhipuai.ZhipuAI(**client_params).embeddings
        return values
    ...
    def _get_len_safe_embeddings(
        ...
        This method handles tokenization and embedding generation, respecting the
        set embedding context length and chunk size.
        ...
        if self.show_progress_bar:
            try:
                ...
                _iter: Iterable = tqdm(range(0, len(texts)))
            except ImportError:
                _iter = range(0, len(texts))
        else:
            _iter = range(0, len(texts))
    ...
    async def _aget_len_safe_embeddings(
        ...
        """Zhipuai has not implemented the asynchronous embedding interface
        """
        return self._get_len_safe_embeddings(texts=texts, engine=engine, chunk_size=chunk_size)
